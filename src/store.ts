	...
	actions: {
		fetchStats: async ({ commit }) => {
			...
			const [fetchedDownloads, fetchedStars, fetchedContributors] = await Promise.all([
				fetch('https://api.npmjs.org/downloads/range/2013-08-21:2100-08-21/discord.js-cluster').then(toJSON, noop),
				fetch('https://api.github.com/repos/MrGriefs/discord.js-cluster').then(toJSON, noop),
				fetch('https://api.github.com/repos/MrGriefs/discord.js-cluster/stats/contributors').then(toJSON, noop),
			]);
